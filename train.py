...
def trainer(config, model, train_loader, valid_loader):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', factor=0.5, patience=5, verbose=True)  # ReduceLROnPlateau : loss 변화량에 기반해 lr 조절
    ...
    for epoch in epochs:
        ...
        print(
            f"epoch : {epoch}, train_loss : {np.mean(loss_arr)}, valid_loss : {eval_loss}")
